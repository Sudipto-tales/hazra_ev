        ...
        if (cfg.mount && cfg.fields && !cfg.el) {
            ...
            if (mountEl) {
                const fieldsHtml = cfg.fields.map(f => {
                    ...
                    const helpHtml = f.help ? `<small style="font-size:11px;color:var(--text-muted);display:block;margin-top:4px;">${U.esc(f.help)}</small>` : '';
                    let inputHtml = '';
                    const isWide = f.wide === true || (f.wide !== false && (f.type === 'wysiwyg' || f.type === 'editor' || f.type === 'textarea' || f.type === 'image' || f.type === 'media' || f.name === 'content' || f.name === 'excerpt' || f.name === 'cover_image' || f.name === 'hero_image' || f.name === 'meta_description'));
                    const colSpanStyle = isWide ? 'grid-column: 1 / -1;' : '';
                    const fieldClass = isWide ? 'field field--wide' : 'field';
                    ...
                    if (f.type === 'wysiwyg' || f.type === 'editor') {
                        ...
                        return root.HAZRA.fields ? root.HAZRA.fields.media(Object.assign({ hint: f.help }, f)) : `
                            <div class="${fieldClass}" style="${colSpanStyle}">
                                ${label}
                        ...
                        return `<div class="${fieldClass}" style="${colSpanStyle}">
                            <label class="checkbox" style="display:flex;align-items:center;gap:8px;cursor:pointer;padding-top:14px;min-height:42px;">
                ...
                mountEl.innerHTML = `
                    <form id="autoGeneratedForm" novalidate style="width:100%;max-width:100%;box-sizing:border-box;padding:28px;background:var(--surface-1);border:1px solid var(--hairline);border-radius:18px;" class="card">
                        <div class="form-grid" style="gap:20px 24px;">
                            ${fieldsHtml}
                        </div>
