...
<div class="download-hero">
    ...
    <?php if ($release): ?>
        ...
        <a href="<?= base_url('/api/v1/app-releases/' . $release['id'] . '/download') ?>" class="btn-download">
            <i class="fa fa-download"></i> Download APK
        </a>
        
        <div class="download-meta">
            <?php if (!empty($release['file_size_bytes'])): ?>
                <span class="badge"><?= round($release['file_size_bytes'] / 1048576, 1) ?> MB</span>
            <?php endif; ?>
            
            <?php if (!empty($release['file_hash'])): ?>
                <span>SHA-256: <?= e(substr($release['file_hash'], 0, 16)) ?>...</span>
            <?php endif; ?>
        </div>
    <?php else: ?>
        <h1>Hazra EV App</h1>
